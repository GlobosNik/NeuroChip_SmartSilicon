...
─────
  Interactive mode  (prompts for paths):
      python neurochip_analyzer.py

  Command-line mode:
      python neurochip_analyzer.py --verilog path/to/circuit.v      \
                                   --top-k   10                     \\
                                   --save-report report.json

  Batch mode (multiple files):
      python neurochip_analyzer.py --verilog a.v b.v c.v            \
...
NUM_GRAPH_FEATURES = 6          # signal_map_len, num_gates, num_inputs, num_outputs,
...
# GNN Model — identical architecture to trainer
...
class FaultGNN(nn.Module):
    ...
    Stage 2 — Dual global pooling fused with circuit-level context features,
               fed into a 3-layer MLP classifier.
    """
    def __init__(self,
...
# 3.  Fault-Suggestion Knowledge Base
...
def parse_verilog(filepath: str) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    """
    ...
    edge_df : DataFrame — one row per signal connection (src → dst)
        columns: src, dst
    meta    : dict — module_name, num_inputs, num_outputs, num_gates,
                     num_wires, source_lines (list of (lineno, text) for annotated output)
    """
    ...
    for lineno, raw_line in enumerate(lines, start=1):
        ...
        # We allow optional strength/delay annotations and multi-line ports.
        gate_matched = False
...
def build_pyg_data(node_df: pd.DataFrame,
                   edge_df: pd.DataFrame,
                   meta: dict) -> Data | None:
    """
    Convert parsed Verilog DataFrames to a PyG Data object using exactly the
    same feature encoding as the training pipeline.
    ...
    graph_feats = torch.tensor([[
        ...
        avg_deg,
        0.0,  # max_depth (not computed; 0 is safe — model was trained with this)
    ]], dtype=torch.float)
...
def visualize_circuit(node_df: pd.DataFrame,
                      edge_df: pd.DataFrame,
                      suspicious_ids: list[int],
                      module_name: str,
                      output_path: str) -> None:
    """
    Draw the circuit as a directed graph.  Suspicious nodes are outlined in
    bright red and enlarged; other nodes are coloured by gate type.
    """
    G = nx.DiGraph()
    for _, row in node_df.iterrows():
    ...
    # Layout: use hierarchical if graph is a DAG, else spring
    try:
...
def annotated_source(source_lines: list[tuple],
                     suspicious: list[dict],
                     output_path: str) -> None:
    """
    Write a copy of the Verilog source with ⚠ markers beside suspicious lines.
    """
    lineno_to_rank: dict[int, int] = {}
    for node in suspicious:
        if node["line_no"] > 0:
            lineno_to_rank.setdefault(node["line_no"], node["rank"])

    with open(output_path, "w") as fh:
        fh.write(f"# NeuroChip annotated source — {datetime.now():%Y-%m-%d %H:%M}\n")
        for lineno, text in source_lines:
            if lineno in lineno_to_rank:
                rank = lineno_to_rank[lineno]
                fh.write(f"{lineno:4d}  [rank {rank}]  {text}\n")
            else:
                fh.write(f"{lineno:4d}             {text}\n")
...
def analyze(verilog_path: str,
    ...
    # ── Parse ────────────────────────────────────────────────────────────────
    node_df, edge_df, meta = parse_verilog(verilog_path)
    ...
    # ── Build PyG graph ───────────────────────────────────────────────────────
    data = build_pyg_data(node_df, edge_df, meta)
    if data is None:
    ...
    if save_graph:
        ...
        visualize_circuit(node_df, edge_df, susp_ids,
                          meta["module_name"], graph_path)

    # ── Save annotated source ─────────────────────────────────────────────────
    annotated_path = None
    if save_annotated and meta.get("source_lines"):
        annotated_path = f"{stem}_annotated.v"
        annotated_source(meta["source_lines"], suspicious, annotated_path)
    ...
    return {
        ...
        "annotated_source":  annotated_path,
...
def print_report(report: dict, verbose: bool = True) -> None:
    ...
    if report.get("annotated_source"):
        print(f"  {DIM}Annotated .v → {report['annotated_source']}{RESET}")
    print(f"\n{'═'*65}")



# 10.  Interactive Prompt Mode
...
def main():
    ...
    parser.add_argument("--no-annotated", action="store_true",
                        help="Skip saving the annotated Verilog source")
